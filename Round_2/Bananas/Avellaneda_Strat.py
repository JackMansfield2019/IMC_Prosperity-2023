...
def BananaStrategy(self: Strategy, state: TradingState) -> None:
    ...
    lim = 0
    if len(self.data['price_history']) < Vol_look_back:
        lim = len(self.data['price_history'])
    else:
        lim = Vol_look_back
    vol = statistics.stdev(self.data['price_history'][-lim:])*math.sqrt(lim)

    T = 1000000
    t = state.timestamp 

    Reservation_Price = s - q*gamma*math.pow(vol,2)*(T-t)
    Spread = gamma*math.pow(vol)(T-t)+(2/gamma)*numpy.log(1+(gamma/kappa))
    
    AP = Reservation_Price
    BP = Reservation_Price - (Spread/2)
