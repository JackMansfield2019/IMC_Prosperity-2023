...
# Dict of product to its correlating product
...
def pairsTradingStrategy(self: Strategy, state: TradingState, correlating_symbol: Symbol, correlating_product_limit: Position) -> None:
    ...
    corr_df = pd.DataFrame({'price': self.data['price_history'], 'correlating_product_price': self.data['correlating_product_price_history']})
...
for product in correlations:
    strategies.append(Strategy(product, limits[product], lambda self, state: pairsTradingStrategy(self, state, correlations[product], limits[correlations[product]])))
