    ...
    if correlation != 0:
        if correlation < LOWER_CORR_THRESHOLD and not self.data["in_trade"]:
            ...
            if self.data['macd'][-1] > 0:
                print("Selling", self.symbol, "at", getMidPrice(state, self.symbol), "because of correlation", correlation, "and macd", self.data['macd'][-1])
                self.addMarketOrders(order_depth_after_mkt_orders, state.position[self.symbol], False, 9999999)
            elif self.data['macd'][-1] < 0:
                print("Buying", self.symbol, "at", getMidPrice(state, self.symbol), "because of correlation", correlation, "and macd", self.data['macd'][-1])
