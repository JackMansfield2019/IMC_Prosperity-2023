...
def get_EMA_slope(self: Strategy, state: TradingState, L: int,long : bool = True) -> float:
    '''
...
def BananaStrategy(self: Strategy, state: TradingState) -> None:
    ...
    add_EMA(self, state, 12.0, self.data['ema_short'])
    ...
    ema_slope_short = get_EMA_slope(self, state, 12,False)
    ema_slope_Long = get_EMA_slope(self, state, 96)
    ...
    print(str(highest_bid) + " " + str(lowest_ask) + " " + str(our_lowest_ask) + " " + str(our_highest_bid))
